...
LINKS_API  = "https://www.mediafire.com/api/1.5/file/get_links.php?quick_key={key}&link_type=direct_download&response_format=json"
...
async def _get_json(session: aiohttp.ClientSession, url: str, timeout=None) -> dict:
    """Fetch URL and return parsed JSON. Retries on failure."""
...
async def _get_text(session: aiohttp.ClientSession, url: str, timeout=None) -> str:
    """Fetch URL and return text. Retries on failure."""
    last_exc = Exception("Unknown error")
    t = timeout or TIMEOUT_SHORT
    for attempt in range(1, MAX_RETRIES + 1):
        try:
...
async def get_info(url: str) -> Optional[dict]:
    ...
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        ...
        if key:
            try:
                data = await _get_json(session, FILE_API.format(key=key))
                if data.get("response", {}).get("result") == "Success":
                    fi = data["response"]["file_info"]
                    ldata = await _get_json(session, LINKS_API.format(key=key))
                    links  = ldata.get("response", {}).get("links", [])
                    dl_url = ""
                    if isinstance(links, list) and links:
                        dl_url = (
                            links[0].get("direct_download")
                            or links[0].get("normal_download", "")
                        )
...
async def download(
    ...
    Uses a fresh session with no total timeout so large files complete fully.
    """
    last_exc = Exception("Download failed")
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            async with aiohttp.ClientSession(headers=HEADERS) as session:
                async with session.get(url, timeout=TIMEOUT_DL, allow_redirects=True) as resp:
                    resp.raise_for_status()
